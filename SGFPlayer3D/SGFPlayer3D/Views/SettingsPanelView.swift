            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Picker("", selection: $ogsMode) {
                            Text("Local").tag(false)
                            Text("OGS").tag(true)
                        }
                        .pickerStyle(.segmented)
                        .onAppear {
                            app.setOGSMode(ogsMode)
                        }
                        .onChange(of: ogsMode) { _, newValue in
                            app.setOGSMode(
                                newValue,
                                autoStartLocal: autoStartOnLaunch,
                                randomOnStart: UserDefaults.standard.bool(forKey: "randomOnStart")
                            ) { autoNext = $0 }
                        }

                        Text(ogsMode ? "Play live games on Online Go Server" : "Play local SGF game files")
                            .font(.caption)
                            .foregroundColor(.secondary)
